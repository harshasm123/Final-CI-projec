...
AMPLIFY_URL=$(aws cloudformation describe-stacks --stack-name pharma-ci-frontend-dev-$TIMESTAMP --query 'Stacks[0].Outputs[?OutputKey==`AmplifyURL`].OutputValue' --output text --region us-east-1)

echo ""
echo "✅ Stacks deployed with Amplify!"
